...
//基于websocket version 13
    ...
    function encode($str) {
        $info = (array) json_decode($str);
        if (!empty($info)) {
            $id = $this->straddcslashes($info["id"]);
            $inputarea = $this->straddcslashes($info["inputarea"]);
            $sql = "select * from alice.u_user where loginname='" . $id . "' ;";


            return $this->strstripslashes('<div >' . $id . ': ' . $inputarea . '</div>');
        }
    }
